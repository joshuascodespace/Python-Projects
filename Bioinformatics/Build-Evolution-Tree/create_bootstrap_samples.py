import sys
import numpy as np
# Used chatgpt to navigate the file/folder i/o for this problem
import os
...
def create_samples():
    new_genes = []
    bootstrap_formula = np.random.randint(0, n, size=n)  # random initial assignment
    for gene in original:
        new_gene = ""
        for letter in range(len(gene)):
            new_gene += gene[bootstrap_formula[letter]]
        new_genes.append(new_gene)
    return new_genes


for sample in range(n_samples):
    gene_count = len(original)
    genes = create_samples()
    genetic_distances = []
    ...
    edges_file = os.path.join(folder_path, 'edges_' + str(sample) + '.txt')
    ...
    while(gene_count >= 3):
        ...
        # Need to make sure that distance matrix is redefined before each iteration of loop.