...
def get_WPB(sample, tree_info):
    ...
    # Maps species name to line in edge file.
    info_indices = {}
    ...
    # Complicated: Basically, ensures that the largest node's group's weight is accounted for accurately, since the weight it actually gets is dominated by the second most abundant node.
    full_weight_species = species_by_abundance[0]
    while (full_weight_species != root_node):
        edge_weights[full_weight_species] = 1 # Arbitrary start value. 
        full_weight_species = tree_info[info_indices[full_weight_species]][0]
    ...
    for i, species in enumerate(species_by_abundance):
        ...
        for node in new_nodes:
            if not node in edge_weights.keys():
