...
def get_WPB(sample, tree_info):
    ...
    # Allows for a species to eadily be accessed within the tree info.
    #info_indices = []
    #for i in range(species_by_abundance):
    #    for j in range(len(tree_info)):
    #        if tree_info[j][1] == species_by_abundance[i]:
    #            info_indices.append(j)

    # Alternative code for the above that's more efficient:
    # tree_info = sorted(tree_info, key=lambda x: x[1])
    ...
    for i, species in enumerate(species_by_abundance):
        ...
        while(True):
            ...
            if len(descendants) == 2 or all(encountered_species_are_in_descendants):
                tree_contribution += float(tree_info[info_indices[current_node]][2]) * sample[species]
                if len(descendants) == 2:
                    ...
                    child = children[0]
                while(True):
                    tree_contribution += float(tree_info[info_indices[child]][2]) * sample[species]
