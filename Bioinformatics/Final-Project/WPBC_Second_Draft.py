...
def get_WPB(sample, tree_info):
    ...
    for data in tree_info:
        ...
        descendants_abundance[parent] += descendants_abundance[child]
    ...
    for i, species in enumerate(species_by_abundance):
        ...
        while(True):
            ...
            if len(descendants) == 2 or all(encountered_species_are_in_descendants):
                tree_contribution += float(tree_info[info_indices[current_node]][2]) * descendants_abundance[current_node]
                if len(descendants) == 2:
                    ...
                    child = children[0]
                while(True):
                    tree_contribution += float(tree_info[info_indices[child]][2]) * descendants_abundance[child]
